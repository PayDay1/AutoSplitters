...
init
{
    // Exclude script from running if you open the Legendary edition of the game, which has the .exe named the same
    ...
    vars.watchers.Add(new MemoryWatcher<byte>(new DeepPointer(modules.First().BaseAddress + 0xE7F010, 0x11C, 0x4D0, 0x3C, 0x14A)) { Name = "plotPrologue" });    // Used for prologue mission
    vars.watchers.Add(new MemoryWatcher<byte>(new DeepPointer(modules.First().BaseAddress + 0xE7F010, 0x11C, 0x4D0, 0x3C, 0x43))  { Name = "plotCR0" });         // CR0, CR1, CR2 and CR3 are the CRitical missions that drive the story forward (Freedom's progress, Horizon, Collector Ship and Normany crew abduction)
    vars.watchers.Add(new MemoryWatcher<byte>(new DeepPointer(modules.First().BaseAddress + 0xE7F010, 0x11C, 0x4D0, 0x3C, 0x27))  { Name = "plotCR123" });       // CR0, CR1, CR2 and CR3 are the CRitical missions that drive the story forward (Freedom's progress, Horizon, Collector Ship and Normany crew abduction)
    vars.watchers.Add(new MemoryWatcher<byte>(new DeepPointer(modules.First().BaseAddress + 0xE7F010, 0x11C, 0x4D0, 0x3C, 0xB3))  { Name = "plotIFF" });         // Corresponds to Legion acquisition, but that is one and the same with the reaper IFF mission
    vars.watchers.Add(new MemoryWatcher<byte>(new DeepPointer(modules.First().BaseAddress + 0xE7F010, 0x11C, 0x4D0, 0x3C, 0x12E)) { Name = "crewAbduct" });      // Used in place of CR3 for "reasons"
    vars.watchers.Add(new MemoryWatcher<byte>(new DeepPointer(modules.First().BaseAddress + 0xE7F010, 0x11C, 0x4D0, 0x3C, 0xFE))  { Name = "suicideOculus" });   // Suicide mission: destroying the oculus
    vars.watchers.Add(new MemoryWatcher<byte>(new DeepPointer(modules.First().BaseAddress + 0xE7F010, 0x11C, 0x4D0, 0x3C, 0x16F)) { Name = "suicideValve" });    // Suicide mission: making it through the ventilation system
    vars.watchers.Add(new MemoryWatcher<byte>(new DeepPointer(modules.First().BaseAddress + 0xE7F010, 0x11C, 0x4D0, 0x3C, 0x170)) { Name = "suicideBubble" });   // Suicide mission: making it through the biotic bubble part
    vars.watchers.Add(new MemoryWatcher<byte>(new DeepPointer(modules.First().BaseAddress + 0xE7F010, 0x11C, 0x4D0, 0x3C, 0x1BC)) { Name = "suicideReaper" });   // Used for signalling the end of the game

    // Dossiers
    vars.watchers.Add(new MemoryWatcher<byte>(new DeepPointer(modules.First().BaseAddress + 0xE7F010, 0x11C, 0x4D0, 0x3C, 0x278)) { Name = "crewAcq1" });        // For Mordin, Jack, Garrus, Tali
    vars.watchers.Add(new MemoryWatcher<byte>(new DeepPointer(modules.First().BaseAddress + 0xE7F010, 0x11C, 0x4D0, 0x3C, 0x4D))  { Name = "crewAcq2" });        // For Grunt
    vars.watchers.Add(new MemoryWatcher<byte>(new DeepPointer(modules.First().BaseAddress + 0xE7F010, 0x11C, 0x4D0, 0x3C, 0x279)) { Name = "crewAcq3" });        // For Thane
    vars.watchers.Add(new MemoryWatcher<byte>(new DeepPointer(modules.First().BaseAddress + 0xE7F010, 0x11C, 0x4D0, 0x3C, 0x47))  { Name = "crewAcq4" });        // For Samara
    vars.watchers.Add(new MemoryWatcher<byte>(new DeepPointer(modules.First().BaseAddress + 0xE7F010, 0x11C, 0x4D0, 0x3C, 0x32))  { Name = "crewAcq5" });        // For Zaeed
    vars.watchers.Add(new MemoryWatcher<byte>(new DeepPointer(modules.First().BaseAddress + 0xE7F010, 0x11C, 0x4D0, 0x3C, 0xB9))  { Name = "crewAcq6" });        // For Kasumi

    // Loyalty missions
    vars.watchers.Add(new MemoryWatcher<byte>(new DeepPointer(modules.First().BaseAddress + 0xE7F010, 0x11C, 0x4D0, 0x3C, 0xBB)) { Name = "loyaltyMissions1" }); // Loyalty mission status for Miranda, Jacob, Jack, Legion, Kasumi, Garrus, Thane and Tali
    vars.watchers.Add(new MemoryWatcher<byte>(new DeepPointer(modules.First().BaseAddress + 0xE7F010, 0x11C, 0x4D0, 0x3C, 0xBC)) { Name = "loyaltyMissions2" }); // Loyalty mission status for Mordin, Grunt, Samara/Morinth and Zaeed

    // N7 Missions
    vars.watchers.Add(new MemoryWatcher<byte>(new DeepPointer(modules.First().BaseAddress + 0xE7F010, 0x11C, 0x4D0, 0x3C, 0x1A8)) { Name = "WMF" });             // N7: Wrecked Merchant Freighter
    vars.watchers.Add(new MemoryWatcher<byte>(new DeepPointer(modules.First().BaseAddress + 0xE7F010, 0x11C, 0x4D0, 0x3C, 0x225)) { Name = "ARS" });             // N7: Abandoned Research Station
    vars.watchers.Add(new MemoryWatcher<byte>(new DeepPointer(modules.First().BaseAddress + 0xE7F010, 0x11C, 0x4D0, 0x3C, 0x18C)) { Name = "ADS" });             // N7: Archeological Dig Site
    vars.watchers.Add(new MemoryWatcher<byte>(new DeepPointer(modules.First().BaseAddress + 0xE7F010, 0x11C, 0x4D0, 0x3C, 0x156)) { Name = "MSVE" });            // N7: MSV Estevanico
    vars.watchers.Add(new MemoryWatcher<byte>(new DeepPointer(modules.First().BaseAddress + 0xE7F010, 0x11C, 0x4D0, 0x3C, 0x28A)) { Name = "ESD" });             // N7: Eclipse Smuggling Depot (and N7: Endangered Research Station)
    vars.watchers.Add(new MemoryWatcher<byte>(new DeepPointer(modules.First().BaseAddress + 0xE7F010, 0x11C, 0x4D0, 0x3C, 0x288)) { Name = "LO" });              // N7: Lost Operative

    // X, Y and Z positions
    vars.watchers.Add(new MemoryWatcher<uint>(new DeepPointer(modules.First().BaseAddress + 0xE8312C, 0x0, 0x4C, 0x110)) { Name = "XPOS" });
    vars.watchers.Add(new MemoryWatcher<uint>(new DeepPointer(modules.First().BaseAddress + 0xE8312C, 0x0, 0x4C, 0x114)) { Name = "YPOS" });
    vars.watchers.Add(new MemoryWatcher<uint>(new DeepPointer(modules.First().BaseAddress + 0xE8312C, 0x0, 0x4C, 0x118)) { Name = "ZPOS" });
...
start
{
    ...
    return (vars.watchers["XPOS"].Old == 1136428027 && vars.watchers["YPOS"].Old == 3338886377 && (vars.watchers["XPOS"].Changed || vars.watchers["YPOS"].Changed));
}
